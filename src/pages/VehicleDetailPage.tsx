...
import { useGetServiceRecords, useCreateServiceRecord, useUpdateServiceRecord, useDeleteServiceRecord } from '../gql/hooks/serviceRecordHooks';
    ...
    <div className="tw:max-w-3xl tw:mx-auto tw:px-4 tw:py-6">
      ...
      <div className="tw:bg-[#1a0f2e] tw:rounded-lg tw:border tw:border-[rgba(188,19,254,0.3)] tw:p-5 tw:mb-6">
        ...
        <div className="tw:grid tw:grid-cols-2 tw:gap-x-6">
          <div>
            <InlineField label="Year" value={vehicle.year} type="number" onSave={(v) => handleUpdate('year', v)} />
            <InlineField label="Make" value={vehicle.make} onSave={(v) => handleUpdate('make', v)} />
            <InlineField label="Model" value={vehicle.model} onSave={(v) => handleUpdate('model', v)} />
            <InlineField label="Mileage" value={vehicle.mileage} type="number" onSave={(v) => handleUpdate('mileage', v)} />
            <InlineField label="Color" value={vehicle.color} onSave={(v) => handleUpdate('color', v)} placeholder="Not set" />
          </div>
          <div>
            <InlineField label="Trim" value={vehicle.trim} onSave={(v) => handleUpdate('trim', v)} placeholder="Not set" />
            <InlineField label="Plate" value={vehicle.plate} onSave={(v) => handleUpdate('plate', v)} placeholder="Not set" />
            <InlineField label="VIN" value={vehicle.vin} onSave={(v) => handleUpdate('vin', v)} placeholder="Not set" />
            <InlineField label="Insurance ID" value={vehicle.insuranceId} onSave={(v) => handleUpdate('insuranceId', v)} placeholder="Not set" />
          </div>
        </div>
      </div>
            ...
            <div className="tw:flex tw:items-start tw:justify-between tw:gap-3">
              <div className="tw:flex-1 tw:min-w-0">
                ...
                <InlineField
                  label="Date"
                  value={record.date}
                  onSave={(v) => updateRecord.mutate({ id: record.id, vehicleId: vehicle.id, date: v })}
                />
                <InlineField
                  label="Mileage"
                  value={record.mileage}
                  type="number"
                  onSave={(v) => updateRecord.mutate({ id: record.id, vehicleId: vehicle.id, mileage: Number(v) })}
                />
                <InlineField
                  label="Cost"
                  value={record.cost}
                  type="number"
                  onSave={(v) => updateRecord.mutate({ id: record.id, vehicleId: vehicle.id, cost: v ? Number(v) : undefined })}
                  placeholder="Not set"
                />
                <InlineField
                  label="Name"
                  value={record.name}
                  onSave={(v) => updateRecord.mutate({ id: record.id, vehicleId: vehicle.id, name: v || undefined })}
                  placeholder="None"
                />
                <InlineField
                  label="Description"
                  value={record.description}
                  onSave={(v) => updateRecord.mutate({ id: record.id, vehicleId: vehicle.id, description: v || undefined })}
                  placeholder="None"
                />
              </div>
...
function AddServiceRecordModal({ vehicleId, currentMileage, onClose, onSave, isPending }: {
  vehicleId: string;
  currentMileage: number;
  onClose: () => void;
  onSave: (data: { vehicleId: string; type: ServiceRecordType; date: string; mileage: number; cost?: number; name?: string; description?: string }) => void;
  isPending: boolean;
}) {
