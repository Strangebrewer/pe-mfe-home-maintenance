    ...
    <div className="tw:max-w-4xl tw:mx-auto tw:px-4 tw:py-6">
        ...
        <div className="tw:bg-surface tw:rounded-lg tw:border tw:border-purpleBorder">
          ...
          {sortedTasks.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className="tw:flex tw:items-center tw:gap-3 tw:px-4 tw:py-3 tw:border-b tw:border-purpleFaint tw:last:border-b-0"
              >
                <div className="tw:flex-1 tw:min-w-0">
                  <span className="tw:text-sm tw:font-medium tw:text-primary">{task.name}</span>
                ...
                <button
                  onClick={() =>
                    setLogCompletion({ taskId: task.id, homeId: task.homeId, taskName: task.name })
                  }
                  className="tw:text-xs tw:bg-transparent tw:border tw:border-green tw:text-green tw:hover:bg-green tw:hover:text-bg tw:rounded tw:px-2 tw:py-1 tw:shrink-0"
                >
                  Log
                </button>
              </div>
      ...
      {showAddVehicle && (
        <AddVehicleModal
          ...
          onSave={(data) =>
            createVehicle.mutate(data, { onSuccess: () => setShowAddVehicle(false) })
          }
        ...
        <LogCompletionModal
          ...
          onSave={(date, cost, notes) =>
            logCompletion_.mutate(
              { taskId: logCompletion.taskId, date, cost, notes },
  ...
  return (
    <Modal title="Add Home" onClose={onClose}>
      <form onSubmit={handleSubmit} className="tw:flex tw:flex-col tw:gap-4">
        ...
        <div>
          <label className="tw:block tw:text-sm tw:font-medium tw:text-muted tw:mb-1">Notes</label>
          <textarea
            value={notes}
  ...
  return (
    <Modal title="Add Vehicle" onClose={onClose}>
      <form onSubmit={handleSubmit} className="tw:flex tw:flex-col tw:gap-4">
        ...
        <div className="tw:grid tw:grid-cols-3 tw:gap-3">
          <div>
            <label className="tw:block tw:text-sm tw:font-medium tw:text-muted tw:mb-1">Trim</label>
          ...
          <div>
            <label className="tw:block tw:text-sm tw:font-medium tw:text-muted tw:mb-1">VIN</label>
...
function LogCompletionModal({
  ...
  return (
    <Modal title={`Log: ${taskName}`} onClose={onClose}>
      <form onSubmit={handleSubmit} className="tw:flex tw:flex-col tw:gap-4">
        <div>
          <label className="tw:block tw:text-sm tw:font-medium tw:text-muted tw:mb-1">Date *</label>
          <input
            type="date"
            value={date}
        ...
        <div className="tw:flex tw:justify-end tw:gap-2 tw:pt-1">
          ...
          <button
            type="submit"
            disabled={isPending}
            className="tw:text-sm tw:border tw:border-green tw:text-green tw:hover:bg-green tw:hover:text-bg tw:disabled:opacity-50 tw:rounded tw:px-4 tw:py-2"
          >
